...
We have taken a single transformation measure, which is dummy coding the categorical attributes
...
## Trivial baseline with a single train/dev split
...
`{r} # creating a mean trivial baseline is not possible with the caret package # the mean trivial baseline will be created with a single train/dev split  mean_score_train <- mean(train_dummy_train$score)}`

### Trivial baseline (mean) evaluation
...
`{r} # training a linear model using k-fold crossvalidation lm <- train(score ~ ., data=train_dummy, method="lm", trControl=train_control)}`
...
`{r} # training a model averaged neural network using k-fold crossvalidation nn <- train(score ~ ., data=train_dummy, method="avNNet", trControl=train_control)}`
...
`{r} # training knn using k-fold crossvalidation knn <- train(score ~ ., data=train_dummy, method="knn", trControl=train_control)}`
...
## Other models trained with a single train/dev split
